...
pair<int,int> getGenotype(int idx,int *gt_arr)
{
    pair<int,int> ret;
    if(idx<0)
    {
    ...
    {
        ret.first = bcf_gt_allele(gt_arr[idx * 2]);
        ret.second = bcf_gt_allele(gt_arr[idx * 2 + 1]);
    }
    return(ret);
}
...
Genotype::Genotype(int idx,int *gt_arr)
{
    if(idx<0)
    {
	_g0=-1;
    ...
    {
	setGenotype(gt_arr[2*idx],gt_arr[2*idx+1]);
    }
}
