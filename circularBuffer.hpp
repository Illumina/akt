...
public:
    ...
    int *getGT(int i)
    { return (_gt[(_offset + i) % _bufsize]); }

    float getMAF(int i)
    { return (_maf[(_offset + i) % _bufsize]); }
    ...
    bcf1_t *getLine(int i)
    { return (_line[(_offset + i) % _bufsize]); }

//set stuff
    int setFilter(int i)
    {
        _keep[(_offset + i) % _bufsize] = false;
        return (0);
    }

    int setKeep(int i)
    {
        _keep[(_offset + i) % _bufsize] = true;
        return (0);
    }
...
//check stuff
    bool isFull()
    { return (_nsnp == _bufsize); }

    bool isFiltered(int i)
    { return (!_keep[(_offset + i) % _bufsize]); }
