...
int admix_main(int argc,char **argv) {
	...
	readMatrix(in_cfile2, prop_data, prop_lab, to_string(d+2) + "-" + to_string(2*d+1) );
	in_cfile2.close();
	
	if(mu_data.size() < d || mu_data.size() > d+1){ 
		cerr << "For " << d << " dimensional data need at " << d << " or " << d+1 << " input vectors" << endl; 
		exit(1); 
	}	
