...
bcf1_t *bcf_copy_sans_format(bcf1_t *dst, bcf1_t *src)
{
    ...
    dst->rlen = src->rlen;
    dst->qual = src->qual;
    dst->n_info = src->n_info;
    dst->n_allele = src->n_allele;
    dst->n_fmt = 0;
    dst->n_sample = 0;
...
int mendel(args &a)
{
    ...
    if (a.outfile != NULL)
    {
        ...
        bcf_hdr_append(out_hdr,
                       "##INFO=<ID=DUO,Number=9,Type=Integer,Description=\"parent-child duo genotype counts\">");
        bcf_hdr_append(out_hdr,
                       "##INFO=<ID=TRIO,Number=27,Type=Integer,Description=\"mother-father-child trio genotype counts\">");
    ...
    vector<vector<vector<vector<int> > > > inheritance_counts(nsample, vector<vector<vector<int> > >(3,
                                                                                                     vector<vector<int> >(
                                                                                                             3,
                                                                                                             vector<int>(
                                                                                                                     3,
    ...
    cout
            << "PED_ID\tCHILD_ID\tDAD_ID\tMUM_ID\tDAD_GT\tMUM_GT\tCHILD_RR\tCHILD_RA\tCHILD_AA\tNERROR\tERROR_RATE\tHET_RATE"
            << endl;
        ...
        if (ped.dadidx[i] != -1 || ped.mumidx[i] != -1)
        {
            ...
            if (ped.dadidx[i] != -1 && ped.mumidx[i] != -1)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    {
                        int num = 0, den = 0;
                        cout << i << "\t" << ped.id[i] << "\t" << ped.id[ped.dadidx[i]] << "\t" << ped.id[ped.mumidx[i]]
                             << "\t" << look[j] << "\t" << look[k];
                        for (int l = 0; l < 3; l++)
                        {
                        ...
                        cout << "\t" << num << "\t" << std::setw(10) << (float) num / (float) den << "\t"
                             << std::setw(10) << (float) inheritance_counts[i][j][k][1] / (float) den << endl;
                    }
                }
            } else if (ped.dadidx[i] != -1)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    cout << "\t" << num << "\t" << std::setw(10) << (float) num / (float) den << "\t" << std::setw(10)
                         << (float) inheritance_counts[i][j][0][1] / (float) den << endl;
                }
            } else if (ped.mumidx[i] != -1)
            {
                for (int k = 0; k < 3; k++)
                {
                    ...
                    cout << "\t" << num << "\t" << std::setw(10) << (float) num / (float) den << "\t" << std::setw(10)
                         << (float) inheritance_counts[i][0][k][1] / (float) den << endl;
                }
            }
