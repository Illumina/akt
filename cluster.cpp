...
void Cluster::clusterAssign(){
	...
	for(int i=0; i<N; ++i){
		...
		for(int j=0; j<K; ++j){
			...
			if( dist < min_dist ){ min_dist = dist; c = j; }
		}
		assignment(i) = c;
		++sizes(c);
	} 
...
float Cluster::EMweights(){
	...
	//calc weights
	for(int k=0; k<K; ++k){
		MatrixXf MS = P.transpose(); for(int n=0; n<N; ++n){ MS.col(n) -= centres.row(k); }	//MS.col(i) = x_i - mu_k
...
void Cluster::EMcluster(int max_its){
		...
		for(int it=0; it<max_its; ++it){
			...
			for(int k=0; k<K; ++k){ 
				...
				for(int n=0; n<N; ++n){
					covariance[k].noalias() += weight(k,n) * (P.row(n) - centres.row(k)).transpose() * (P.row(n) - centres.row(k));
				} 
			...
			if( abs(newP - oldP)/abs(oldP) < eps ){ cerr << "Clustering completed after " << it << " iterations" << endl; break; }
...
static void usage(){ 
    ...
    cerr << "\t -a --alg:			clustering algorithm 0 = k++means, 1 = gaussian mixture, 2 = density method" << endl;
    umessage('c');
...
int cluster_main(int argc,char **argv) {
    ...
    int K = 2;
    int alg = 0;
    bool use_file = false; string cfile = "";
    string dims="";
    float rho_min = 1;
    ...
    while ((c = getopt_long(argc, argv, "k:a:c:C:I:d:p:D:i:eo:",loptions,NULL)) >= 0) {  
        ...
        case 'd': dc = atof(optarg); break;
        case 'p': rho_min = atof(optarg); break;
        case 'D': delta_min = atof(optarg); break;
        case 1: density_plot = true; break;
        case 'i': seed = atoi(optarg); break;
        case 'e': dosil = true; break;
        case 'C': use_file = true; cfile = (optarg); break;
        case 'c': dims = (optarg); break;
        case 'o': output_c = (optarg); break;
        case '?': usage();
        default: die("Unknown argument:"+(string)optarg+"\n");
        }
    }
	if(dims==""){ cerr << "-c argument is mandatory" << endl; exit(1); }
	switch(alg){
		case 0:
			cerr << "K++ means" << endl; break;
		case 1:
			cerr << "EM Gaussian" << endl; break;
		case 2:
			cerr << "Density Clustering" << endl; break;
	}
	optind++;
